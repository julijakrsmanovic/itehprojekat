...
export class addStatusToRelationship1632169183725 implements MigrationInterface {
    name = 'addStatusToRelationship1632169183725'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("ALTER TABLE `relationship` CHANGE `accepted` `status` tinyint NOT NULL");
        ...
        await queryRunner.query("ALTER TABLE `relationship` DROP COLUMN `status`");
        await queryRunner.query("ALTER TABLE `relationship` ADD `status` varchar(255) NOT NULL");
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("ALTER TABLE `post` ADD CONSTRAINT `FK_5c1cf55c308037b5aca1038a131` FOREIGN KEY (`userId`) REFERENCES `user`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION");
        await queryRunner.query("ALTER TABLE `relationship` DROP COLUMN `status`");
        await queryRunner.query("ALTER TABLE `relationship` ADD `status` tinyint NOT NULL");
        await queryRunner.query("ALTER TABLE `message` CHANGE `senderId` `senderId` int NULL DEFAULT 'NULL'");
        await queryRunner.query("ALTER TABLE `message` ADD CONSTRAINT `FK_bc096b4e18b1f9508197cd98066` FOREIGN KEY (`senderId`) REFERENCES `user`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION");
        await queryRunner.query("ALTER TABLE `relationship` CHANGE `status` `accepted` tinyint NOT NULL");
    }
